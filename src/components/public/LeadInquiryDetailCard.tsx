...
export const LeadInquiryDetailCard: React.FC<LeadInquiryDetailCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-indigo-200/90 shadow-sm overflow-hidden text-slate-900 animate-in fade-in duration-200 space-y-5 p-5">
      ...
      {/* 6. GHI CHÚ VẬN HÀNH & TÀI LIỆU ĐÍNH KÈM TỪ KHÁCH HÀNG */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Left: Customer Instruction Notes */}
        <div className="p-4 rounded-2xl bg-amber-50/50 border border-amber-200/80 space-y-2.5 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-black uppercase text-amber-900 tracking-wider pb-2 border-b border-amber-200/60">
              <FileText className="w-4 h-4 text-amber-700" />
              <span>Ghi Chú Vận Hành Chi Tiết Từ Khách Hàng (Operating Notes)</span>
            </div>
            <p className="text-xs text-slate-700 leading-relaxed bg-white/90 p-3.5 rounded-xl border border-amber-100 italic mt-2.5 shadow-2xs">
              "{specs.customerNotes}"
            </p>
          </div>
          {lead.contractTerm && (
            <div className="pt-2 text-[11px] text-amber-800 font-semibold flex items-center gap-1.5">
              ...
              <span>Thời hạn cam kết: <strong>{lead.contractTerm}</strong></span>
            </div>
          )}
        </div>

        {/* Right: Attached Files & Drawings */}
        <div className="p-4 rounded-2xl bg-slate-50/90 border border-slate-200 space-y-2.5">
          ...
          <div className="space-y-2 pt-0.5">
            {specs.attachments.map((file, fIdx) => (
              <div
                key={fIdx}
                className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-slate-200/80 hover:border-indigo-300 transition-colors text-xs shadow-2xs"
              >
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-[11px] font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 active:bg-indigo-200 rounded-lg transition-colors flex items-center gap-1 shrink-0 cursor-pointer shadow-2xs"
                >
