        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-black text-slate-900 tracking-tight">Studio Hồ Sơ Saleman & Landing Page</span>
              <span className="px-2 py-0.5 bg-indigo-100 text-indigo-800 text-[10px] font-extrabold rounded-md uppercase">
                TopCV Style
              </span>
            </div>
            <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
              <span>Đang dùng mẫu:</span>
              <strong className="text-slate-800 font-bold flex items-center gap-1">
                {templateNames[config.activeTemplateId]}
              </strong>
              <span>•</span>
              ...
              <div className="absolute right-0 mt-2 w-72 bg-white rounded-2xl shadow-2xl border border-slate-200 p-3 z-30 animate-in fade-in zoom-in-95 duration-100">
                ...
                <div className="mt-2 space-y-3 max-h-72 overflow-y-auto pr-1">
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    ...
                    <div className="space-y-1 text-xs">
                      {[
                        { key: 'highlights', label: '1. Thước đo quy mô & năng lực' },
                        { key: 'branches', label: '2. Mạng lưới chi nhánh & văn phòng' },
                        { key: 'affiliations', label: '3. Hiệp hội & Mạng lưới toàn cầu' },
                        { key: 'ecosystem', label: '4. Hệ sinh thái dịch vụ & Ngành hàng' },
                        { key: 'partners', label: '5. Đối tác chiến lược & Khách hàng' },
                        { key: 'caseStudies', label: '6. Dự án & Case studies thành công' },
                      ].map((item) => (
                        <label key={item.key} className="flex items-center justify-between p-1.5 hover:bg-slate-50 rounded-lg cursor-pointer">
                          <span className="text-slate-700 font-medium">{item.label}</span>
                          <input 
