...
export const WarehousingInquiryForm: React.FC<WarehousingInquiryFormProps> = ({
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200">
      ...
      {/* 2.5 Thông Số Vận Hành Đặc Thù Kho Lạnh & Kiểm Soát Độ Ẩm (Chỉ hiển thị khi là Kho Lạnh hoặc Hàng Lạnh) */}
      {(cargoClassification === 'Reefer' || specs.warehouseType === 'Kho lạnh / Kho mát (Cold Storage)') && (
        <div className="p-4 bg-cyan-50/60 border border-cyan-200/90 rounded-2xl space-y-3 shadow-2xs animate-in fade-in duration-150">
          <div className="flex items-center justify-between">
            <span className="text-xs font-extrabold text-cyan-950 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span>Yêu Cầu Vận Hành Đặc Thù Kho Lạnh & Kiểm Soát Độ Ẩm</span>
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
            ...
            <div>
              ...
              <select
                value={specs.humidityRequirement || 'Không yêu cầu kiểm soát độ ẩm đặc biệt (Chuẩn kho lạnh/mát)'}
                onChange={(e) => updateSpec('humidityRequirement', e.target.value)}
                className="w-full px-3 py-2.5 text-xs bg-white border border-cyan-300 rounded-xl focus:border-cyan-500 font-bold text-cyan-950 shadow-2xs cursor-pointer"
              >
              ...
              {specs.humidityRequirement === 'Tùy chỉnh riêng (% RH)' && (
                <div className="mt-2">
                  <input
                    type="text"
                    value={specs.customHumidity || ''}
                    onChange={(e) => updateSpec('customHumidity', e.target.value)}
                    placeholder="VD: Duy trì ổn định 50% - 60% RH"
                    className="w-full px-3 py-2 text-xs bg-white border border-cyan-300 rounded-xl focus:border-cyan-500 font-semibold text-slate-800"
                  />
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => updateSpec('inboundTemperatureState', 'PRE_COOLED')}
                  className={`p-2.5 rounded-xl border text-left cursor-pointer transition-all ${
                    specs.inboundTemperatureState !== 'NEED_COOLING'
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border ${
                      specs.inboundTemperatureState !== 'NEED_COOLING' ? 'border-cyan-600 bg-cyan-600 text-white' : 'border-slate-300'
                    }`}>
                      {specs.inboundTemperatureState !== 'NEED_COOLING' && <CheckCircle2 className="w-2.5 h-2.5" />}
                    </span>
                    <span className="text-xs font-bold">Hàng đã đạt chuẩn</span>
                  </div>
                  <p className="text-[10px] text-slate-500 mt-1 pl-5 font-normal">
                    Đã hạ nhiệt/cấp đông trước khi nhập kho (Pre-cooled).
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => updateSpec('inboundTemperatureState', 'NEED_COOLING')}
                  className={`p-2.5 rounded-xl border text-left cursor-pointer transition-all ${
                    specs.inboundTemperatureState === 'NEED_COOLING'
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border ${
                      specs.inboundTemperatureState === 'NEED_COOLING' ? 'border-amber-600 bg-amber-600 text-white' : 'border-slate-300'
                    }`}>
                      {specs.inboundTemperatureState === 'NEED_COOLING' && <CheckCircle2 className="w-2.5 h-2.5" />}
                    </span>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1 pl-5 font-normal">
                    Hàng tươi mới cần hạ nhiệt/cấp đông gió (Blast Freezing).
                  </p>
                </button>
              </div>
            </div>
